...
% fs_opt = 0.045102342; % old
fs_opt = 0.037003410; % new
...
[fx, gradf, ~, hessf] = Oracles(b, A, sigma, h);
...
e.NM = compute_error(A_test,b_test,x.NM);
fprintf('Error w.r.t 0-1 loss: %1.3e\n',e.NM);
end
...
e.QNM = compute_error(A_test,b_test,x.QNM);
fprintf('Error w.r.t 0-1 loss: %1.3e\n',e.QNM);
end

%%%% STOCHASTIC METHODS
[fx, gradf, gradfsto, ~] = Oracles(b, A, sigma, h);
...
if isfield(info,'NM')
loglog(info.NM.fx-fs_opt, 'LineWidth',3,'color',colors(9,:)); hold on;
legend_lab{end+1} = 'NM';
end
if isfield(info,'QNM')
loglog(info.QNM.fx-fs_opt, 'LineWidth',3,'color',colors(10,:),'LineStyle',':'); hold on;
legend_lab{end+1} = 'QNM';
end
