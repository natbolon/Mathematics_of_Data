...
% Compute the Magnitude of each image
f_abs = sqrt(real(f).^2 + imag(f).^2);
f_wav_abs = sqrt(real(F).^2 + imag(F).^2);
x_rec_abs = sqrt(real(x_rec_lin).^2 + imag(x_rec_lin).^2);

% Compute PSNR 
psnr_linear = abs(psnr(f_abs, x_rec_abs));
psnr_wav    = abs(psnr(f_abs, f_wav_abs));
...
fig         = figure;
fontsize    = 16;
imagesc(abs(F), [min(abs(F(:))), max(abs(F(:)))]),  axis image off;
