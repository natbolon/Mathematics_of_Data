...
file_name  = 'Images-33/2/psnr-regularizer';
saveas(fig, file_name, 'epsc');

%% VISUALIZE results
[v,i] = max(PSNR_list);

regs_2 = [1e-8,regs(i), 0.5]
for k=1:length(regs_2)
    regularization_parameter_lasso = regs_2(k)
    % Define operators
    fx          = @(x) 0.5*norm( b(1:p) - real(forward_operator(x(1:N))) + imag(forward_operator(x(N+1:end))),2)^2 + ...
                   0.5*norm(b(p+1:end) - imag(forward_operator(x(1:N))) - real(forward_operator(x(N+1:end))),2)^2;
    ...
    % Compute PSNR
    PSNR = abs(psnr(sqrt(real(f).^2 + imag(f).^2), sqrt(real(F).^2 + imag(F).^2)))
    ...
    imagesc(abs(F),[min(abs(F(:))), max(abs(F(:)))])
    t = strcat(strcat('Regularizer = ', num2str(regularization_parameter_lasso)), strcat(' PSNR = ', num2str(PSNR)));
    title(t,'fontsize',16,'interpreter','latex');
    file_name  = strcat('Images-33/2/', num2str(k));
    saveas(fig, file_name, 'epsc');
%     
%     % Set warm start
%     x0 = f_wav; 
